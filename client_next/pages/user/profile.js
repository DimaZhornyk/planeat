...
const QUERY = gql`
  query getRecipes($slugs: [String!]) {
    recipes(where: { slugs: $slugs }) {
      id
      slug
      recipeImage {
        url
...
function Profile({ recipes, categories }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (recipes !== undefined) {
        ...
        if (recipes.length !== 0)
          return Client.query({ query: QUERY, variables: recipeId }).then(
            ({ data }) => data.recipes
          );
        return undefined;
      }
  ...
  const displayRecipe =
    likeRecipes === undefined
      ? ""
      : likeRecipes.map((recipe, index) => (
          <Col xl={8} lg={8} md={12} sm={12} xs={24} key={index}>
            <RecipeCard
              id={recipe.id}
              image={recipe.recipeImage.url}
              caption={recipe.recipeCaption}
              time={recipe.time}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      <Header categories={categories} styles={{ marginBottom: "50px" }} />
      <div className={styles["main-page-wrapper"]}>
        <Row gutter={[16, 16]}>
          {likeRecipes !== undefined ? (
            ...
            <Empty
              description={"Немає обраних рецептів..."}
