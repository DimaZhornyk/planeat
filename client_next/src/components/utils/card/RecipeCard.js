...
function RecipeCard(props) {
  ...
  const getOptions = (options) => {
    if (options.length > optionsLength) {
      ...
      optionsArray.push(
        <CustomOptionCard
          key={"more"}
          item={options.length - optionsLength + 1}
        />
      );
    } else {
  ...
  return (
    <>
      <Card
        hoverable={true}
        style={{ borderRadius: "10px", height: "320px", cursor: "default" }}
        key={props.id}
        bodyStyle={{ padding: "0px", borderRadius: "10px", height: "120px" }}
        className={"product-card"}
      >
        <Image
          src={MAIN_BACKEND_URL + props.image}
            ...
            {props.isAuth && (
              <Button
                ...
                onClick={() => {
                  if (
                    props.ids === null ||
                    props.ids.indexOf(props.slug) === -1
                  ) {
                    return props.addRecipes(props.slug).then(console.log);
                  }
